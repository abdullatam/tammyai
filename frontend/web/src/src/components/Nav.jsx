...
const Nav = ({ active = 'chat', onNavigate, userInitial = 'T' }) => {
  ...
  const followupCount = window.TammyData?.followup_count || 0;
  const unreadCount  = window.TammyData?.unread_count || 0;
  // Map screen id → badge count
  const badges = { today: followupCount, chat: unreadCount };
  const items = [
  ...
  return (
    ...
    >
      ...
      <div
        ...
        alignItems: 'stretch',
        gap: 8,
        transition: `width 380ms ${EASE}, border-radius 380ms ${EASE}, box-shadow 380ms ${EASE}`,
        overflow: 'hidden',
      }}>
        {items.map(item => {
