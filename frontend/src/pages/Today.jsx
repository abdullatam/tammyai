...
const TodayScreen = ({ onNavigate, activeBucket, setActiveBucket, tweaks, updateTweaks }) => {
      ...
      <section style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingRight: 8 }}>
          <div className="mono" style={{ fontSize: 11, color: 'var(--ink-3)', textTransform: 'uppercase', letterSpacing: '0.16em', marginBottom: 14 }}>
            {D.greeting?.phrase || 'hey'} <span style={{ color: 'var(--ink-2)' }}>{D.user?.name ? D.user.name.toLowerCase() : ''}</span>
          </div>
          <h1 className="serif" style={{
            ...
            {D.greeting?.hero_line || "what's on your chest?"}
          </h1>
          {D.greeting?.subtext && (
