...
const HealthPage = () => {
  ...
  useEffect(() => {
    ...
    const sse = new EventSource(`/admin/api/costs/live?token=${encodeURIComponent(token)}`);
    sse.onmessage = (e) => {
      try {
  ...
  const fetchHealth = useCallback(async () => {
    setBusy(true);
    try {
      const [svcRes, dbRes, pcRes, llmRes, errRes, redisRes, voiceRes] = await Promise.all([
        ...
        window.AdminAPI._fetch('/admin/api/health/voice').catch(() => null)
      ]);
      setHealthData({
  ...
  * { box-sizing: border-box; }
  ...
  .billing-card:hover { border-color: var(--mauve); transform: translateY(-2px); box-shadow: 0 12px 30px rgba(31,28,48,0.08); }
  .billing-card::before {
    content: '';
    position: absolute;
  ...
  .billing-card.anthropic::before { background: linear-gradient(90deg, #C88BFF, #9B6EE8); }
  .billing-card.openai::before    { background: linear-gradient(90deg, #74B8FF, #3D8FE0); }

  .billing-card .bc-head {
    display: flex; align-items: center; justify-content: space-between;
    margin-bottom: 20px;
  }
  .billing-card .bc-name {
  ...
  .billing-card .bc-spend {
    display: flex; align-items: baseline; gap: 4px;
    margin-bottom: 4px;
  }
  .billing-card .bc-spend .dollar {
    font-family: var(--f-mono); font-size: 13px;
    color: var(--ink-3); font-weight: 400;
  }
  .billing-card .bc-spend .amount {
    font-family: var(--f-mono); font-size: 36px;
    font-weight: 700; color: var(--ink);
    letter-spacing: -0.04em; line-height: 1;
  }
  .billing-card .bc-spend .period {
    font-family: var(--f-mono); font-size: 11px;
    color: var(--ink-3); letter-spacing: 0.08em;
    text-transform: uppercase;
  }

  .billing-card .bc-proj {
    font-family: var(--f-mono); font-size: 12px;
    color: var(--ink-3); margin-bottom: 20px;
    letter-spacing: 0.04em;
  }
  .billing-card .bc-proj span {
    color: var(--warn); font-weight: 600;
  }

  .billing-card .bc-stats {
    display: grid; grid-template-columns: 1fr 1fr;
    gap: 10px;
    padding-top: 16px;
    border-top: 1px dashed var(--mauve-soft);
  }
  .billing-card .bc-stat {
    display: flex; flex-direction: column; gap: 3px;
  }
  .billing-card .bc-stat .sk {
    font-family: var(--f-mono); font-size: 9px;
    letter-spacing: 0.18em; text-transform: uppercase;
    color: var(--ink-3);
  }
  .billing-card .bc-stat .sv {
    font-family: var(--f-mono); font-size: 18px;
    font-weight: 600; color: var(--ink);
    letter-spacing: -0.02em;
  }
  .billing-card .bc-stat .sv em {
  ...
  .billing-total {
    ...
    flex-wrap: wrap;
    box-shadow: 0 4px 20px rgba(31,28,48,0.06);
  }
  .billing-total .bt-label {
    font-family: var(--f-mono); font-size: 10px;
    letter-spacing: 0.2em; text-transform: uppercase;
    color: var(--ink-3); margin-bottom: 4px;
  }
  .billing-total .bt-val {
    font-family: var(--f-mono); font-size: 28px;
    font-weight: 700; color: var(--amber);
    letter-spacing: -0.03em;
  }
  .billing-total .bt-meta {
    font-family: var(--f-mono); font-size: 10px;
  ...
  .bc-remain-val {
    font-family: var(--f-mono); font-size: 44px;
  ...
  /* ── Burn bar ── */
  .bc-bar-wrap { margin-bottom: 20px; }
  .bc-bar-track {
    height: 5px;
  ...
  .bc-bar-fill {
    height: 100%;
    border-radius: 999px;
    transition: width 800ms cubic-bezier(0.32,0.72,0.24,1);
...
`}</style>
      <div className="page health-page">
            ...
            <div key={s.id} className={`svc ${s.status === 'bad' ? 'bad-svc' : s.status === 'warn' ? 'warn-svc' : ''}`}>
              <div className="row1">
                <div>
                  ...
                  {s.credits && <div className="provider" style={{ color: 'var(--amber)', marginTop: 6, fontWeight: 500 }}>Balance: {s.credits}</div>}
                </div>
                <div className="stat">
        ...
        {/* Voice APIs */}
        <div className="section-h">
          <h2>Voice Synthesis (TTS)</h2>
          <span className="h-meta">live account limits</span>
        </div>
        <div className="db-grid">
          <div className="db">
            <div className="head">
              <div className="name">
                <span className={`stat-dot ${healthData?.voice?.elevenlabs?.status === 'ok' ? 'ok' : 'bad'}`} aria-hidden="true"></span>
                ElevenLabs Account
              </div>
              <span className="h-meta">{healthData?.voice?.elevenlabs?.tier || 'unknown'}</span>
            </div>
            <div className="row">
              <span className="k">Remaining Chars</span>
              <span className="v">
                <span className="num" style={{ color: 'var(--amber)', fontWeight: 600 }}>
                  {(Number(healthData?.voice?.elevenlabs?.charsLimit || 0) - Number(healthData?.voice?.elevenlabs?.charsUsed || 0)).toLocaleString()}
                </span> 
                <em> / {Number(healthData?.voice?.elevenlabs?.charsLimit || 0).toLocaleString()}</em>
              </span>
            </div>
            <div className="row">
              <span className="k">Custom Voices</span>
              <span className="v num">
                {healthData?.voice?.elevenlabs?.voicesUsed || 0} <em>/ {healthData?.voice?.elevenlabs?.voicesLimit || 0}</em>
              </span>
            </div>
            <div className="row">
              <span className="k">Instant Cloning</span>
              <span className="v">{healthData?.voice?.elevenlabs?.canInstantClone ? 'Enabled' : 'Disabled'}</span>
            </div>
          </div>
          ...
          <div className="db">
            <div className="head">
              <div className="name">
                <span className={`stat-dot ${healthData?.voice?.speechmatics?.status === 'ok' ? 'ok' : 'bad'}`} aria-hidden="true"></span>
                Speechmatics Account
              </div>
              <span className="h-meta">asr usage</span>
            </div>
            <div className="row">
              <span className="k">Audio Processed</span>
              <span className="v">
                <span className="num">{(Number(healthData?.voice?.speechmatics?.hoursProcessed || 0) * 60).toFixed(1)}</span>
                <em> mins</em>
              </span>
            </div>
            <div className="row">
              <span className="k">Jobs Completed</span>
              <span className="v num">{Number(healthData?.voice?.speechmatics?.jobsProcessed || 0).toLocaleString()}</span>
            </div>
            <div className="row">
              <span className="k">Tracking Since</span>
              <span className="v">{healthData?.voice?.speechmatics?.since || 'n/a'}</span>
            </div>
          </div>
        </div>

        {/* Voice Pipeline (Chain Grid) */}
        ...
        {/* Live AI Billing Intelligence — Wallet Style */}
        <div className="section-h" style={{ marginTop: '30px' }}>
          <h2>Live AI billing intelligence</h2>
          <span className="h-meta">
            <span className="billing-live-dot">streaming · 5s interval</span>
          </span>
        </div>

        <div className="billing-grid">
          {/* Anthropic Claude wallet card */}
          {(() => {
            const d = costData?.anthropic || {};
            const start   = d.starting_balance || 3.29;
            const life    = d.lifetime_spend   || 0;
            const remain  = d.remaining_balance ?? start;
            const pct     = d.pct_used || 0;
            const barW    = Math.min(100, pct);
            const barColor = pct > 80 ? 'var(--bad)' : pct > 50 ? 'var(--warn)' : 'var(--ok)';
            return (
              <div className="billing-card anthropic">
                <div className="bc-head">
                  <div className="bc-name">
                    <span className={`stat-dot ${d.status || 'ok'}`} aria-hidden="true"></span>
                    Anthropic Claude
                  </div>
                  <div className="bc-badge">pay-as-you-go</div>
                </div>

                {/* Remaining balance — main hero number */}
                <div className="bc-remain-label">remaining credit</div>
                <div className="bc-remain">
                  <span className="bc-remain-sym">$</span>
                  <span className="bc-remain-val num">{remain.toFixed(2)}</span>
                  <span className="bc-remain-of">/ ${start.toFixed(2)}</span>
                </div>

                {/* Burn bar */}
                <div className="bc-bar-wrap">
                  <div className="bc-bar-track">
                    <div className="bc-bar-fill" style={{ width: barW + '%', background: barColor }}></div>
                  </div>
                  <div className="bc-bar-meta">
                    <span style={{ color: barColor }}>{pct.toFixed(1)}% used</span>
                    <span>${life.toFixed(4)} spent total</span>
                  </div>
                </div>

                <div className="bc-stats">
                  <div className="bc-stat">
                    <div className="sk">today's spend</div>
                    <div className="sv num">${(d.daily_spend || 0).toFixed(4)}</div>
                  </div>
                  <div className="bc-stat">
                    <div className="sk">30-day projection</div>
                    <div className="sv num">${(d.projected_monthly || 0).toFixed(2)}</div>
                  </div>
                  <div className="bc-stat">
                    <div className="sk">req / min</div>
                    <div className="sv num">{d.rpm || 0}<em> rpm</em></div>
                  </div>
                  <div className="bc-stat">
                    <div className="sk">tokens / min</div>
                    <div className="sv num">{Number(d.tpm || 0).toLocaleString()}<em> tpm</em></div>
                  </div>
                </div>
              </div>
            );
          ...
          {/* OpenAI wallet card */}
          {(() => {
            const d = costData?.openai || {};
            const start   = d.starting_balance || 3.07;
            const life    = d.lifetime_spend   || 0;
            const remain  = d.remaining_balance ?? start;
            const pct     = d.pct_used || 0;
            const barW    = Math.min(100, pct);
            const barColor = pct > 80 ? 'var(--bad)' : pct > 50 ? 'var(--warn)' : 'var(--ok)';
            return (
              <div className="billing-card openai">
                <div className="bc-head">
                  <div className="bc-name">
                    <span className={`stat-dot ${d.status || 'ok'}`} aria-hidden="true"></span>
                    OpenAI GPT
        ...
        <div className="billing-total">
          <div>
            <div className="bt-label">total remaining</div>
            <div className="bt-val num" style={{ color: (costData?.total_remaining || 6.36) < 1 ? 'var(--bad)' : 'var(--ok)' }}>
              ${(costData?.total_remaining ?? (3.07 + 3.29)).toFixed(2)}
            </div>
            <div className="bt-meta">openai + anthropic wallets</div>
          </div>
          <div>
            <div className="bt-label">total spent (all time)</div>
            <div className="bt-val num">
              ${((costData?.anthropic?.lifetime_spend || 0) + (costData?.openai?.lifetime_spend || 0)).toFixed(4)}
            </div>
            <div className="bt-meta">since tracking started</div>
          </div>
          <div>
            <div className="bt-label">today's spend</div>
            <div className="bt-val num">${(costData?.total_spend || 0).toFixed(4)}</div>
            <div className="bt-meta">projected ${(costData?.total_projected || 0).toFixed(2)}/mo</div>
          </div>
          <div>
            <div className="bt-label">live throughput</div>
            <div className="bt-val num">
              {(costData?.anthropic?.rpm || 0) + (costData?.openai?.rpm || 0)}<span style={{fontSize:14,fontWeight:400,color:'var(--ink-3)',marginLeft:4}}>rpm</span>
            </div>
            <div className="bt-meta">{Number((costData?.anthropic?.tpm || 0) + (costData?.openai?.tpm || 0)).toLocaleString()} tpm</div>
          </div>
        </div>
